...
def _run_experiment(dataset, n_experiments=3, n_medoids=5, useCacheP=True, usePerm=True, cacheMultiplier=5000):
...
def run_experiment(dataset_name, n_experiments, n_data, n_medoids):
    print(f"\n[{dataset_name}={n_data}, K={n_medoids}]")
    dataset = get_dataset(dataset_name=dataset_name, n_data=n_data)
    stats1 = _run_experiment(dataset, n_experiments=n_experiments, n_medoids=n_medoids, useCacheP=False, usePerm=False)
    stats2 = _run_experiment(dataset, n_experiments=n_experiments, n_medoids=n_medoids, useCacheP=True, usePerm=False)
    stats3 = _run_experiment(dataset, n_experiments=n_experiments, n_medoids=n_medoids, useCacheP=True, usePerm=True)
    stats = [stats1, stats2, stats3]
    return stats

def run_experiments(dataset_list=[], n_data_list=[], n_medoids_list=[]):
    stats_list = []
    ...
    for dataset_name in dataset_list:
        for n_data in n_data_list:
            for n_medoids in n_medoids_list:
                stats = run_experiment(dataset_name=dataset_name, n_experiments=3, n_data=n_data, n_medoids=n_medoids)
                stats_list += stats,
...
def main(argv):
    try:
        ...
        run_experiments(dataset_list, n_data_list, n_medoids_list)
