...
def main():
    ...
    def test_cache_stats():
        ...
        time_1000, width_1000, writes_1000, hits_1000, misses_1000 = get_cache_statistics(kmed=kmed, X=X, loss="L2", cache_width=1000)
        time_750, width_750, writes_750, hits_750, misses_750 = get_cache_statistics(kmed=kmed, X=X, loss="L2", cache_width=750)
        time_500, width_500, writes_500, hits_500, misses_500 = get_cache_statistics(kmed=kmed, X=X, loss="L2", cache_width=500)
        time_250, width_250, writes_250, hits_250, misses_250 = get_cache_statistics(kmed=kmed, X=X, loss="L2", cache_width=250)
        time_0, width_0, writes_0, hits_0, misses_0 = get_cache_statistics(kmed=kmed, X=X, loss="L2", cache_width=0)

        assert hits_1000 > hits_750 > hits_500 > hits_250 > hits_0, "Cache hits should increase as cache size increases"
        assert misses_1000 < misses_750 < misses_500 < misses_250 < misses_0, "Cache misses should decrease as cache size increases"
        assert misses_1000 == 0, "There should be no cache misses at width=1000 as the whole dataset fits in memory"
        assert hits_0 == 0, "There should be no cache hits at width=0 as there is no cache"
        assert writes_0 == 0, "There should be no cache writes at width=0 as there is no cache"
    ...
    def test_old_bpam_vs_new_bpam():
        X = np.loadtxt(os.path.join("data", "MNIST_1k.csv"))
        kmed = KMedoids(n_medoids=10, algorithm="BanditPAM", use_cache=False, use_perm=False, parallelize=False)
        ...
        kmed_v3 = KMedoids(n_medoids=10, algorithm="BanditPAM_v3", use_cache=False, use_perm=False, parallelize=False)
