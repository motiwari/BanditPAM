    ...
    for x_axis in x_axes:
        ...
        for dataset in datasets:
            ...
            for setting in settings:
                # Get BanditPAM original loss first
                algorithm_files = glob.glob(os.path.join(log_dir, f"*{BANDITPAM_ORIGINAL_NO_CACHING}*{dataset}*"
                                                                  f"{setting}*idx*"))
                for algorithm in ALL_BANDITPAMS:
                    if is_multiple_experiments:
                    ...
                    if x_axis == NUM_DATA:
                        ...
                        xtitle = "$n$"
                    title = "$L/L_{BanditPAM}$" + f" vs. {xtitle} {translate_experiment_setting(dataset, setting)}"
                    plt.title(title)
                    plt.xlabel(xlabel)
                    plt.ylabel(ylabel)
...
def create_scaling_plots(
    ...
    for x_axis in x_axes:
        ...
        if dir_name is None:
            ...
            log_dir = os.path.join(parent_dir, "logs", dir_name)
            ...
            for setting in settings:
                for y_axis in y_axes:
                    ...
                    for algorithm in ALL_BANDITPAMS:
                        ...
                        # Calculate the mean of each row across the files
                        # print(setting, algorithm, data.groupby(data.index).std())
                        data_mean = data.groupby(data.index).mean()
                        data_std = data.groupby(data.index).std() / np.sqrt(len(data))
                        # print(data_std)

                        # Drop the 'file_id' column as it's not needed anymore
                        data_mean = data_mean.drop(columns='file_id')
