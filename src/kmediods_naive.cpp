                     ...
                     arma::Row<size_t> &centroid_indicies)
{
    for (size_t k = 0; k < clusters; k++)
    {
        ...
        { //consider each data point as a potentail mediod
            double total = 0;
            for (size_t j = 0; j < data.n_cols; j++)
            {
                ...
                { // or closer mediod and point
                    if (arma::norm(data.col(centroid_indicies(mediod)) - data.col(j), 2) < cost)
                    {
                        cost = arma::norm(data.col(centroid_indicies(mediod)) - data.col(j), 2);
                    }
                }
                total += cost;
            }
                    ...
                    arma::Row<size_t> &centroid_indicies)
{
    ...
    for (size_t k = 0; k < clusters; k++)
    {
        ...
        for (size_t i = 0; i < data.n_cols; i++)
        { //consider each data point as a potential mediod
            double total = 0;
            for (size_t j = 0; j < data.n_cols; j++)
            {
                double cost = arma::norm(data.col(i) - data.col(j), 2); //either add distance between potential mediod and point
