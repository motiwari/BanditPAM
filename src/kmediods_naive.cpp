...
void KMediods::cluster(const arma::mat &data, const size_t clusters,
    ...
    arma::Row<size_t> centroid_indicies(clusters);
    ...
    // build clusters
    KMediods::build(data, clusters, centroid_indicies);
        ...
        for (size_t i = 0; i < data.n_cols;
            ...
            for (size_t j = 0; j < data.n_cols; j++) {
                ...
                for (size_t mediod = 0; mediod < k;
                    ...
                    if (arma::norm(data.col(centroid_indicies(mediod)) -
                                       data.col(j),
                                   2) < cost) {
                        cost = arma::norm(data.col(centroid_indicies(mediod)) -
                                              data.col(j),
                                          2);
                    }
                }
...
void KMediods::swap(const arma::mat &data, const size_t clusters,
    ...
    centroid_indicies(medoid_to_swap) = best;
}
...
double KMediods::calc_loss(const arma::mat &data, const size_t clusters,
    ...
    for (size_t i = 0; i < data.n_cols; i++) {
        ...
        for (size_t mediod = 0; mediod < clusters; mediod++) {
            if (arma::norm(data.col(centroid_indicies(mediod)) - data.col(i),
                           2) < cost) {
                cost = arma::norm(
                    data.col(centroid_indicies(mediod)) - data.col(i), 2);
            }
        }
        total += cost;
    }
