...
void KMedoidsWrapper::predict(const arma::fmat& X_new) {
  ...
  if (n_features != this->getData().n_cols) {
    throw std::runtime_error(
      "Number of features in new data must match training data.");
  }
  ...
  for (size_t i = 0; i < n_new; ++i) {
    ...
    for (size_t j = 0; j < n_medoids; ++j) {
      ...
      if (this->getLossFn() == "L1" || this->getLossFn() == "manhattan") {
      ...
      } else if (this->getLossFn() == "L2" ||
                 this->getLossFn() == "euclidean") {
        for (size_t k = 0; k < n_features; ++k) {
        ...
        for (size_t k = 0; k < n_features; ++k) {
          ...
          norm_medoid +=
            this->getData()(medoid_idx, k) * this->getData()(medoid_idx, k);
        }
        distance =
          1.0 - (dot_product / (std::sqrt(norm_new) * std::sqrt(norm_medoid)));
      }
