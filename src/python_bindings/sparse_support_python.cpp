...
namespace km {
void KMedoidsWrapper::fit_sparse(py::object sparse_matrix,
  ...
  try {
    ...
    py::array_t<float> data =
      csr_matrix.attr("data").cast<py::array_t<float>>();
    py::array_t<int> indices =
      csr_matrix.attr("indices").cast<py::array_t<int>>();
    py::array_t<int> indptr =
      csr_matrix.attr("indptr").cast<py::array_t<int>>();
    ...
    throw std::runtime_error("Failed to process sparse matrix: " +
                             std::string(e.what()));
  }
}
...
void KMedoidsWrapper::predict_sparse(py::object sparse_matrix_new) {
  if (!this->is_sparse_input) {
    throw std::runtime_error(
      "Model was not fitted with sparse data. Use regular predict() method.");
  }
  ...
  try {
    ...
    bool is_sparse =
      py::cast<bool>(sp_sparse.attr("issparse")(sparse_matrix_new));
    if (!is_sparse) {
      throw std::runtime_error("Input matrix is not a scipy sparse matrix");
    }
    ...
    if (n_cols != this->sparse_n_cols) {
      throw std::runtime_error(
        "Number of features in new sparse data must match training data.");
    }
    ...
    throw std::runtime_error(
      "Failed to process sparse matrix for prediction: " +
      std::string(e.what()));
  }
}
}  // namespace km