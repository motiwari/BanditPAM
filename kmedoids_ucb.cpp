                     ...
                     arma::mat& medoids)
{
    ...
    for (size_t k = 0; k < clusters; k++)
    {
        ...
        while (arma::sum(candidates) > 0.1) {
            ...
            compute_exactly = compute_exactly == exact_mask;
            std::cout << compute_exactly << std::endl;
            ...
            Row<double> result = build_target(data, targets, this_batch_size, best_distances);
            estimates.cols(targets) = ((T_samples.cols(targets) % estimates.cols(targets)) + (result * this_batch_size))
                                        /(this_batch_size + T_samples.cols(targets));
            T_samples.cols(targets) += this_batch_size;
            Row<double> adjust(targets.n_cols);
            adjust.fill(std::log(1/p));
            Row<double> cb_delta = sigma * arma::sqrt(adjust / T_samples.cols(targets));
...
arma::Row<double> KMediods::build_target(const arma::mat& data, arma::uvec& target, size_t batch_size, arma::Row<double>& best_distances ) {
    size_t N = data.n_cols;
    arma::Row<double> estimates(target.n_rows, fill::zeros);
...
double KMediods::cost_fn(const arma::mat &data, arma::uword target,  arma::uvec& tmp_refs, arma::Row<double>& best_distances) {
    double total = 0;
