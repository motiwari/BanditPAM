...
def verify_logfiles():
    ...
    FasterPAM, by parsing the logfiles.

    Note that the same seed must be used for both algorithms to ensure that the
    uniform random sampling is the same.
    """
    ...
    for parent_dir in parent_dirs:
        bfp_logfiles = [
            ...
            if os.path.isfile(os.path.join(parent_dir, x))
            and x != ".DS_Store"
            and x[:5] == "L-bfp"
        ]
        for bfp_lfile in sorted(bfp_logfiles):
            ...
            if not os.path.exists(fp_lfile):
                ...
                if disagreement:
                    ...
                    print(sorted(l2_2))
                    print(
                        "ERROR: Results for", bfp_lfile, fp_lfile, "disagree!!"
                    )
                else:
                    print("OK: Results for", bfp_lfile, fp_lfile, "agree")


def make_plots(loss_dir_name):
    """
    Make a plot showing the relative losses of BanditPAM and FasterPAM,
    normalized to PAM's loss.

    Used for Figure 1(a) of the paper.
    """

    loss_dir = loss_dir_name
        ...
        for algo_idx, algo in enumerate(algos):
            for seed_idx, seed in enumerate(seeds):
                ...
                if not os.path.exists(filename):
                    raise Exception(
                        "Warning: logfile not found for ", filename
                    )
    ...
    for algo_idx, algorithm in enumerate(algos):
        ...
        melt_df["N"] += (
            np.random.randn(melt_df["N"].shape[0]) + this_jitter
        )  # Add jitter

        bars = (1.96 / (10**0.5)) * np.std(
            losses[:, algo_idx, :], axis=1
        )  # Slice a specific algo, get a 2D array
        means = np.mean(losses[:, algo_idx, :], axis=1)
        print(algorithm, this_color, this_label, this_jitter)
        plt.plot(
            np.array(Ns) + this_jitter,
            means,
            color=this_color,
            zorder=this_zorder,
            linewidth=2,
...
if __name__ == "__main__":
    ...
    loss_dir_name = "logs/Loss_plots_paper_manual/"
    make_plots(loss_dir_name)
