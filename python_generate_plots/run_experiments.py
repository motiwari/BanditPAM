...
def write_medoids(
    ...
    medoids after BUILD step (or uniform random sampling), medoids after SWAP
    step, etc.
    """
...
def run_exp(args, object_name, log_fname, output_dir, time):
    ...
    imgs = total_images[
        np.random.choice(
            len(total_images), size=args.sample_size, replace=False
        )
    ]
...
def main(dir_name, experiments):
    ...
    (in Figure 1(a)).
    """
    output_dir = dir_name  # change this based on the experiment
    args = get_args(
        ["run_experiments.py", "-e", "auto_exp_config.py"][1:]
    )  # Uses default values for now as placeholder to instantiate args

    if experiments is None:
        # Use the experiments in the config .py file
        imported_config = importlib.import_module(args.exp_config.strip(".py"))
        loop_over = imported_config.experiments
    else:
        # Use the experiments passed into main()
        loop_over = experiments

    for exp in loop_over:
        ...
        try:
            if exp[0] == "bfp":
                kmed = banditpam.KMedoids(
                    n_medoids=args.num_medoids, algorithm="BanditFasterPAM"
                )
                run_exp(args, kmed, log_fname, output_dir, time=True)
            elif exp[0] == "fp":
                kmed = banditpam.KMedoids(
                    n_medoids=args.num_medoids, algorithm="FasterPAM"
                )
                run_exp(args, kmed, log_fname, output_dir, time=True)
            elif exp[0] == "naive_v1":
                kmed = banditpam.KMedoids(
                    n_medoids=args.num_medoids, algorithm="PAM", max_iter=1
                )
                run_exp(args, kmed, log_fname, output_dir, time=True)
            else:
